...
A recurring hand-stitch motif (`components/ui/StitchLine.tsx`): a straight line of short, even dashes (`9 6` rhythm, round caps) — like a real lock-stitch running through fabric. The line itself is deliberately straight, not hand-wobbled; the "handmade" feeling comes from the dash rhythm and (in the Hero) the needle-pulled draw-in, not from any wobble in the geometry.

Two uses only, kept strictly separate:
- **Animated, once, hero-only**: `<StitchLine animated />` under the Hero eyebrow, in `leather`, sized via the caller's own width class (e.g. Hero's `w-28`) — the component itself applies no default width on this variant, to avoid conflicting with that override. An SVG mask (a classic solid stroke-dashoffset "draw" line) reveals the fixed dash pattern right-to-left over a slow, deliberate `2.2s` glide, matching Hebrew reading direction, via the `stitch-line-animated` class in `globals.css`. A minimal filled needle silhouette (ink, point leading) rides visibly *above* the thread (the `viewBox` has extra vertical room for this) for the whole glide, arriving exactly as the thread finishes — then plunges down in the final ~12% of the timeline to land exactly on the line, with a quick decisive drop and a small overshoot settle (scale bump) selling the impact, before coming to rest at the tip for good — no loop, no idle motion. Respects `prefers-reduced-motion` (shows the fully-drawn thread with the needle already resting on the line at the tip, no animation). This is the one bold/animated moment on the page — do not add the animated variant (or the needle) anywhere else, or it stops being special.
- **Static, quiet, reused as a divider**: plain `<StitchLine />` in `border` color, replacing plain hairline `border-t`/`border-b` section dividers (currently: above `ProductCarousel`'s scroll strip, above `ClutchConfigurator`). No animation, no needle, no leather — it's texture, not an accent.
